...
# genuinely owns: the four patches in `local`, and `README`.
#
...
let
  ...
  # No fork pins here on purpose. Every driver patch and every firmware file
  # in this port comes from the pinned ImmortalWrt revision or from a project
  # ImmortalWrt itself pins (firmware_qca-wireless, linux-firmware,
  # CodeLinaro ath11k-firmware, upstream Linux). Third-party forks of
  # ImmortalWrt are not sources for this port, however convenient an
  # individual patch or firmware image in one of them looks.
in
{
  # The secondary pins above, resolved, so a consumer can fetch from them
  # without repeating a URL: `default.nix` reads wifi.pins.cloAth11k.rawBase
  # for the IPQ6018 firmware, and the port's own list reads
  # wifi.pins.qcaWireless.rawBase for the board data. Every entry under
  # `patch`/`firmware` below names the pin it belongs to.
  pins = {
    inherit linux openwrt qcaWireless cloAth11k;
  };
  ...
  kernel = [
    ...
    }
    {
      ...
      note = ''
        On 6.18 this hunk lands on wcss_ipq8074 instead of wcss_ipq6018, so
        IPQ6018 keeps the invalid "q6wcss" name: 0910 restores both entries.
      '';
    }
    {
    ...
    {
      path = "target/linux/qualcommax/patches-6.18/0909-arm64-dts-qcom-ipq6018-assign-QDSS_AT-clock-to-wifi-.patch";
      blob = "ed6cbd164e0a28c37d9effde585ae2160829c2e6";
      sha256 = "sha256-PcQ/PnVZ+nxYaysZXoaZ0yoCGAPDkeIy/w6rPuW99bk=";
      note = "Gives the wifi remoteproc node the QDSS_AT clock 0908 made optional for it.";
    }
    {
      path = "target/linux/qualcommax/patches-6.18/0910-remoteproc-qcom-q6v5-wcss-fix-ipq6018-ssr-name.patch";
      blob = "8db559d71a909aa58453217e736af9d031219558";
      sha256 = "sha256-0HTdfmJBX/eR5v67oJRYBJUCcp1IQaRkSuh9Mi0qVnw=";
      note = ''
        Sets ssr_name "wcnss" for IPQ6018 and restores "q6wcss" for IPQ8074.
        Without it stopping WCSS sends an unknown name to RPM, which resets the
        SoC into a hung XBL - the AHB radio's stop path hits this.
      '';
    }
  ];
  ...
  reference = {
    ...
    # 0911's command line find/replace hack is not applied: its hunk sits
    # against an fdt.c that carries a "bootargs-append" property, which
    # 6.18.49 does not have, so it would not apply with zero fuzz. What it
    # was wanted for (loglevel/panic on a boot path whose command line the
    # U-Boot web loader replaces) the ath11k-probe service gets from
    # userspace instead.
    kernelCmdline = {
      path = "target/linux/qualcommax/patches-6.18/0911-arm64-cmdline-replacement.patch";
      blob = "f72a29ccd36660fd69ceed19a026d1b05ec8814a";
      sha256 = "sha256-Vtcum4qhnInpyRQSebOYbpOhAY8c/8ltqfnMAfEahfI=";
      why = "Needs the fdt bootargs-append feature, which this kernel does not have.";
    };

    # A local patch this port wrote and then dropped.
    local = [
      {
        file = "971-ath11k-pci-keep-aspm-disabled.patch";
        why = ''
          Both official qualcommax configs keep CONFIG_PCIEASPM_DEFAULT=y, and
          ath11k itself disables ASPM for the whole firmware download
          (ath11k_pci_power_up -> ath11k_pci_aspm_disable). This patch only
          stopped ath11k_pci_start()/ath11k_pci_power_down() from restoring it
          afterwards, i.e. after the MHI stage the QCN9074 was failing in, so
          it could not affect that failure and was a pure deviation.
        '';
      }
    ];
  };
  ...
  firmware = {
    ...
    qcn9074Firmware = {
      ...
      upstreamPin = {
        ...
        hashFrom = "package/firmware/linux-firmware/Makefile at the pin (PKG_VERSION 20260810, PKG_HASH ac17c34f...a49a), converted to SRI.";
      };
      note = ''
        ImmortalWrt's ath11k-firmware-qcn9074 installs the kernel.org
        linux-firmware tree, 20260810 at the pin; OpenWrt master pins
        20260910. This port takes the same two files from nixpkgs'
        linux-firmware, and they are the same bytes as both official pins -
        verified 2026-09-19 against the 20260810 blobs from kernel.org:

          ath11k/QCN9074/hw1.0/amss.bin  063b6adbc9fc66143a828d03b92ac16c2619d02507a945e663ce331cc1128608
          ath11k/QCN9074/hw1.0/m3.bin    3cf2cff8b014986edfd45c595f9702096157e8148d36b51f9a8f71e6c5fb37ba

        (amss.bin is WLAN.HK.2.9.0.1-02146.) So this image ships exactly
        what official ImmortalWrt and official OpenWrt ship, and no
        conclusion here may rest on a different firmware being available.
      '';
    };
  ...
  kconfig = {
    ...
    wlan = {
      ...
      ATH11K_AHB = "m";
      ATH11K_PCI = "m";
      # Debug output for the probe service: ath11k_dbg() exists only with
      # ATH11K_DEBUG, and dynamic debug then drives it from userspace.
      DYNAMIC_DEBUG = "y";
      CFG80211_DEBUGFS = "y";
      MAC80211_DEBUGFS = "y";
      ATH11K_DEBUG = "y";
