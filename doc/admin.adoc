...
Get the image onto the device, somehow

[source,console]
----

build$ cat result/firmware.bin | ssh -e none root@the-device "cat >/run/firmware.bin"
----


Now perform the upgrade, using flashcp. This is an example, your
device will differ
...
# flashcp -v /run/firmware.bin mtd:firmware
----
