...
#   - QCN9074 5GHz 4x4 on PCIe0 (0000:01:00.0), "wlan5g1" here. Off unless
#     wifi.qcn9074.enable is set: on the firmware ImmortalWrt and OpenWrt
#     both ship it does not reach mission mode on this unit (see ./wifi/README).
...
let
  ...
  staticFirmware = pkgs.runCommand "ath11k-firmware" { } (''
    ...
    mkdir -p $root/ath11k/IPQ6018/hw1.0 $root/IPQ6018
  ...
  '' + lib.optionalString cfg.enable ''
    # --- QCN9074 (PCIe 5GHz): the linux-firmware files both official trees
    # ship for this board (see the manifest's hash note).
    mkdir -p $root/ath11k/QCN9074/hw1.0
...
in
{
  ...
  services.firmware-ath11k = pkgs.liminix.services.oneshot {
    ...
    up = ''
      # NB for whoever edits this next: busybox here is `applets =
      # minimalApplets` (modules/busybox.nix) with no per-applet options
      # enabled beyond the few set there, so any applet *option* that is not
      # explicitly enabled is missing. `stat` is in the list but `stat -c` is
      # not - it silently reported every extracted file as "0 bytes" until
      # this was found. Prefer invocations that need no optional feature
      # (`wc -c` rather than `stat -c%s`), and check before adding one.
  ...
  services.ath11k-probe = pkgs.liminix.services.oneshot {
    name = "ath11k-probe";
    # 20s settle + one 30s PCIe wait + the 120s AHB wait
    timeout-up = 240000;
    dependencies = [ config.services.firmware-ath11k ];
    up = ''
      kmsg() { echo "ath11k-probe: $*" > /dev/kmsg; }

      # The web loader replaces /chosen/bootargs, so loglevel= and
      # dynamic_debug= from commandLine never arrive: get both from here.
      echo 8 > /proc/sys/kernel/printk
      mountpoint -q /sys/kernel/debug || mount -t debugfs none /sys/kernel/debug
      if [ -w /sys/module/ath11k/parameters/ath11k_debug_mask ]; then
        echo 0xffffffff > /sys/module/ath11k/parameters/ath11k_debug_mask
      fi
      if [ -w /sys/kernel/debug/dynamic_debug/control ]; then
        for m in ath11k ath11k_pci mhi; do
          echo "module $m +p" > /sys/kernel/debug/dynamic_debug/control
        done
      fi
      ...
      # --- QCN9074 on PCIe0 -------------------------------------------
      #
      # One attempt, deliberately. A QCN9074 whose firmware has already
      # crashed (MHI_CB_EE_RDDM) is not recovered by unbind+rebind: the
      # driver's power_up does a WLAON reset and a PCIe hot reset, but not a
      # power cycle, and every retry after the first died in well under a
      # second on hardware while burying the first failure in RDDM noise.
      # The dmesg from this single attempt is the artefact worth having, so
      # do not manufacture five more.
      pci_ok=0
      kmsg "PCI drivers_probe 0000:01:00.0"
      ...
      if [ $pci_ok -eq 1 ]; then
        ...
        kmsg "QCN9074 up as wlan5g1"
      else
        # Leave the device unbound rather than bound-but-dead: patch 970
        # makes the unbind safe, and the AHB radio still has to be tried.
        kmsg "no netdev from the QCN9074; unbinding and continuing"
        echo 0000:01:00.0 > /sys/bus/pci/drivers/ath11k_pci/unbind 2>/dev/null
      fi
      ''}${lib.optionalString (!cfg.enable) ''
      kmsg "5GHz radio disabled in this build (wifi.qcn9074.enable=false)"
      ''}
  ...
  services.bridge = lib.mkForce (config.system.service.bridge.members.build {
    ...
    ] ++ lib.optionals cfg.enable [ config.services.wlan5g ];
  });
  ...
  services.wlan5g = lib.mkIf cfg.enable (net.link.build {
    ifname = "wlan5g1";
    dependencies = [ config.services.ath11k-probe ];
  });

  services.hostapd-5g = lib.mkIf cfg.enable (config.system.service.hostapd.build {
    ...
    params = {
      ...
      # 11ax stays off: this tree's ath11k was still asserting the firmware
      # on the HE/PEER path under load, which is why the AP is AC-only.
      ieee80211ax = 0;
      auth_algs = 1;
      wpa = 2;
      wpa_key_mgmt = "WPA-PSK";
